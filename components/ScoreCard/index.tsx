...
function ScoreItem({
  ...
  return (
    <div className="space-y-2 border-b border-[#2D3A3A]/10 last:border-0 pb-2 last:pb-0">
      ...
      <button
        type="button"
        className={cn('space-y-1 cursor-default', {
          'cursor-pointer hover:bg-[#2BA84A]/5 rounded-md p-1 -m-1 transition-colors':
            hasIssues,
        })}
        onClick={toggleOpen}
        disabled={!hasIssues}
      >
        <div className="flex justify-between text-sm items-center">
          <div className="flex items-center gap-2">
            <span className="font-medium text-[#2D3A3A] capitalize">
              {label.replace(/_/g, ' ')}
            </span>
            {hasIssues && (
          ...
          <div className="flex items-center gap-2">
            ...
            {hasIssues && (
              <ChevronDown
                className={cn(
                  'h-4 w-4 text-[#2D3A3A]/60 transition-transform duration-200',
                  isOpen && 'rotate-180',
                )}
              />
            )}
      ...
      {hasIssues && isOpen && (
        <div className="mt-2 text-sm text-[#2D3A3A] bg-[#2D3A3A]/5 rounded-md p-3 space-y-3 animate-in fade-in slide-in-from-top-1">
          {issues.map((issue) => (
            <div
              key={`${issue.category}-${issue.recommendation}-${issue.evidence ?? ''}`}
              className="space-y-1"
            >
              {issue.evidence && (
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        <div className="flex flex-col items-center justify-center space-y-2">
          <div className={cn('text-4xl font-bold', overallColor)}>
            {overall_score}/10
          </div>
          ...
          {(Object.entries(scores) as [keyof typeof scores, number][]).map(
            ([key, value]) => {
              ...
              const categoryIssues = issues.filter((i) => {
                ...
                return (
                  issueCat.includes(scoreKey) || scoreKey.includes(issueCat)
                );
              });
